...
// ── SaySee Logo component ────────────────────────────────────────
function SaySeeLogo({height=36}){
  const scale = height / 95;
  const w = Math.round(340 * scale);
  return(
    <svg width={w} height={height} viewBox="0 0 340 95" role="img" aria-label="SaySee">
      <defs>
        <radialGradient id="irisG2" cx="42%" cy="38%" r="60%">
          <stop offset="0%" stopColor="#5BAEE8"/>
          <stop offset="55%" stopColor="#2178C8"/>
          <stop offset="100%" stopColor="#0E5BAD"/>
        </radialGradient>
        <radialGradient id="pupilG2" cx="38%" cy="35%" r="62%">
      ...
      {/* Green smile arc */}
      <path d="M18 52 Q46 76 74 52" fill="none" stroke="#3DAA1E" strokeWidth="6" strokeLinecap="round"/>
      {/* Green tail */}
      <path d="M24 66 Q16 78 10 84 Q20 74 28 68Z" fill="#3DAA1E"/>
      {/* Eye white */}
      <path d="M46 18 Q74 18 86 44 Q74 70 46 70 Q18 70 6 44 Q18 18 46 18Z" fill="#FFFFFF"/>
      {/* Blue upper lid */}
      <path d="M8 36 Q26 16 46 13 Q66 16 84 36" fill="none" stroke="#1565C0" strokeWidth="4.5" strokeLinecap="round"/>
      {/* Blue lower lid */}
      <path d="M8 52 Q26 70 46 73 Q66 70 84 52" fill="none" stroke="#1565C0" strokeWidth="4.5" strokeLinecap="round"/>
      {/* Iris */}
      <circle cx="46" cy="44" r="17" fill="url(#irisG2)"/>
      <circle cx="46" cy="44" r="15.5" fill="none" stroke="#0A4D9A" strokeWidth="1" opacity="0.3"/>
      {/* Pupil */}
      <circle cx="46" cy="44" r="8" fill="url(#pupilG2)"/>
      {/* Catchlight */}
      <circle cx="51" cy="39" r="3.5" fill="#FFFFFF" opacity="0.95"/>
      <circle cx="42" cy="49" r="1.8" fill="#FFFFFF" opacity="0.38"/>

      {/* Say - blue */}
      <text x="100" y="66"
        fontFamily="'Nunito','Arial Rounded MT Bold',sans-serif"
        fontSize="58" fontWeight="900" fontStyle="italic"
        fill="#1565C0">Say</text>
      {/* See - green */}
      <text x="214" y="66"
        fontFamily="'Nunito','Arial Rounded MT Bold',sans-serif"
        fontSize="58" fontWeight="900" fontStyle="italic"
        fill="#3DAA1E">See</text>
    </svg>
  );
}
...
function AuthScreen({accounts,onLogin,onRegister}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0F0C29,#302B63,#24243E)",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"20px 20px",overflowY:"auto"}}>
      <div style={{width:"min(94vw,440px)",paddingTop:"max(20px, 4vh)"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <SaySeeLogo height={48}/>
        </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0F1117",color:"#fff",fontFamily:"'Nunito',sans-serif"}}>
      <header style={{background:"rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.08)",padding:"13px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}><SaySeeLogo height={28}/><span style={{fontSize:12,color:"#A29BFE",fontFamily:"'Nunito',sans-serif",fontWeight:700,background:"#A29BFE22",padding:"2px 8px",borderRadius:20}}>ADMIN</span></div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F4F6FB",fontFamily:"'Nunito',sans-serif"}}>
      ...
      <header style={{background:"#fff",boxShadow:"0 2px 14px rgba(0,0,0,0.07)",padding:"12px 22px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10}}><SaySeeLogo height={28}/><span style={{fontSize:12,color:"#27AE60",fontFamily:"'Nunito',sans-serif",fontWeight:700,background:"#27AE6022",padding:"2px 8px",borderRadius:20}}>DISTRICT</span></div>
