...
function StudentMode({entry,level,listening,transcript,onExit}){
  ...
  return(
    <div onClick={handleTap} style={{position:"fixed",inset:0,background:entry?"#FFFFFF":"#1B4F9E",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24,zIndex:100,userSelect:"none"}}>
      ...
      rec.onresult=(e)=>{
        for(let i=e.resultIndex;i<e.results.length;i++){
          ...
          if(e.results[i].isFinal){
            ...
            // ── 3. Direct word match ──
            const directMatch = wRef.current.find(w=>(w.triggers||[w.word]).some(tr=>t.includes(tr)));
            if(directMatch){
