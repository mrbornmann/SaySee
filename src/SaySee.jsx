...
// ── Master/base words (owner_id NULL, admin-managed, shared to all teachers) ──
const sbWords = {
  getBase: async () => {
    if (!supabase) return null;
    const { data, error } = await supabase.from("words")
      .select("*").is("owner_id", null).eq("is_active", true).order("category").order("sort_order");
    if (error) return null;
    return (data||[]).map(r=>({
  ...
  deleteBase: async (id) => {
    if (!supabase) return;
    const { error } = await supabase.from("words").delete().is("owner_id", null).eq("id", String(id));
    if (error) throw error;
  },
  seedBase: async (words) => {
    if (!supabase) return;
...
Reply with ONLY the matching word or NO_MATCH.`
  ...
  // flag to mute it, but keeping our own audio output genuinely ACTIVE makes the
  // browser duck the earcon. A pure zero-gain node outputs no samples, which newer
  // Chrome no longer treats as "active" (that's why the ding came back) — so we
  // output a continuous, sub-audible, near-zero signal instead: inaudible on real
  // devices, but enough to keep the output stream alive and the earcon suppressed.
  ...
  const startSilent = useCallback(()=>{
    try{
      ...
      if(!s){
        ...
        gain.gain.value = 0.0012;     // ≈ -58 dBFS: inaudible on speakers/headphones, but non-zero output
        const osc  = ctx.createOscillator();
        osc.type = "sine";
        osc.frequency.value = 20;     // sub-audible (≤20 Hz); device speakers can't reproduce it
...
export default function SaySee(){
  ...
  // Load the master word library from Supabase (source of truth). On the first run
  // with an empty table, seed it from the current local/seed list so nothing is lost.
  useEffect(()=>{
    if(!user) return;
    let off=false;
    (async()=>{
      try{
        let base=await sbWords.getBase();
        if(base&&base.length===0&&user.role==="admin"){
          let local;
        ...
        <ErrorBoundary>
          <AdminPanel words={masterWords} setWords={updater=>{
            setMasterWords(prev=>{
              ...
              if(user?.role==="admin"){
                try{
                  const prevById=new Map(prev.map(w=>[String(w.id),w]));
                  const nextIds=new Set(next.map(w=>String(w.id)));
                  next.forEach(w=>{ const p=prevById.get(String(w.id)); if(!p||JSON.stringify(p)!==JSON.stringify(w)) sbWords.upsertBase(w).catch(()=>{}); });
                  prev.forEach(w=>{ if(!nextIds.has(String(w.id))) sbWords.deleteBase(w.id).catch(()=>{}); });
                }catch(e){}
              }
              return next;
            });
