...
// Eye icon only — used in app headers
function SaySeeLogo({size=36}){
  return(
    <img
      src="https://i.postimg.cc/y8QybRqR/Just-Eye.png"
      alt="SaySee"
      style={{
        height:size,
        width:size,
...
// Full square logo — used on login and loading screens
function SaySeeFullLogo({size=160}){
  return(
    <img
      src="https://i.postimg.cc/4NXv4Pst/Logo-Main-See-Say.png"
      alt="SaySee"
      style={{
        height:size,
        width:size,
...
// Banner logo — horizontal, used where there is wide space
function SaySeeBanner({height=50}){
  return(
    <img
      src="https://i.postimg.cc/hPxFgSDH/App-Say-See-Logo.png"
      alt="SaySee — AAC Visual Learning"
      style={{
        height:height,
        width:"auto",
        objectFit:"contain",
...
const MASTER_WORDS = [
  ...
  {id:38,cat:"actions",   word:"go",         display:"GO",           emoji:"🚦", photo:"green traffic light go",           color:"#5AAB2A", triggers:["go","let's go","go ahead"]},
  ...
  return(
    <div onClick={handleTap} style={{position:"fixed",inset:0,background:entry?"#FFFFFF":"#0F0C29",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24,zIndex:100,userSelect:"none"}}>
      <div style={{position:"absolute",top:16,right:18,display:"flex",alignItems:"center",gap:6}}>
        <div style={{width:8,height:8,borderRadius:"50%",background:listening?"#5AAB2A":"#BDC3C7",animation:listening?"mPulse 1.8s ease-in-out infinite":"none"}}/>
        <span style={{fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,color:listening?"#5AAB2A":"#BDC3C7",textTransform:"uppercase",letterSpacing:0.8}}>{listening?"Listening":"Mic off"}</span>
      </div>
          ...
          <div style={{position:"absolute",right:0,top:0,bottom:0,width:"min(94vw,370px)",background:"#fff",overflowY:"auto",animation:"slideR 0.3s ease"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:22}}>
              ...
              {dtab==="words"&&<>
                ...
                <button onClick={()=>setAddWord(true)} style={{width:"100%",padding:"11px",background:"#5AAB2A",color:"#fff",border:"none",borderRadius:12,fontFamily:"'Nunito',sans-serif",fontWeight:900,fontSize:14,cursor:"pointer",marginBottom:14,boxShadow:"0 4px 14px #00B89455"}}>+ Add Custom Word</button>
